...
kable(data_dict, 
      caption = "Specyfikacja zmiennych w zbiorze danych",
      booktabs = TRUE, 
      align = "llll") %>%
...
Przeprowadzona analiza wykazała, że w badanym zbiorze danych występuje **`r n_miss(dane)`** **obserwacji** zawierających braki danych, co stanowi **`r pct_miss(dane)`%** wszystkich obserwacji, natomiast **`r n_complete(dane)`** **obserwacji** stanowią dane kompletne.
...
```{r echo=FALSE, fig.height=12, fig.width=8, warning=FALSE}
...
::: {style="text-align: justify;"}
Otrzymane wyniki analizy wariancji ANOVA wskazują na istotną statystycznie interakcję między porą roku a kategorią produktu. Oznacza to, że wpływ sezonu na wartość zakupów nie jest stały — to, czy średnio od tego, jaką grupę produktów analizujemy. Potwierdza to wizualne obserwacje z wykresu interakcji, gdzie linie trendów przecinały się, zamiast biec równolegle. Brak statystycznie istotnego wpływu zmiennej Season na zmienną Amount, oznacza, że jeśli uśrednimy wydatki ze wszystkich kategorii, pory roku same generują znacząco różnych poziomów sprzedaży. Tak samo brak wpływu zmiennej kategorii na kwotę zakupu, wskazuje, że w skali całego roku, żadna kategoria nie jest droższa lub tańsza od pozostałych.
:::
...
## Analiza zależność między płcią a kategorią zakupionych przedmiotów?
...
-   Większość zmiennych ilościowych wykazuje słabe lub umiarkowane zależności. Szczególnie zauważalna była korelacja między wartością zakupu a oceną produktu oraz między poprzedniki zakupami a wiekiem, co może wskazywać na subtelne wzorce zachowań konsumenckich.

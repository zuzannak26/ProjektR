...
library(gtsummary)
dane_kompletne %>%
  select(Age, Amount, Rating, Previous.Purchases) %>% # Wybierz swoje 3 zmienne ilościowe
  tbl_summary(
    statistic = list(all_continuous() ~ "{mean} ({sd}) [min - max]"),
    digits = list(all_continuous() ~ 2),
    label = list(Age ~ "Wiek", Amount ~ "Kwota", Rating ~ "Ocena", Previous.Purchases ~ "Kwota wydana na wcześniejszych zakupach")
  ) %>%
  bold_labels() %>%
  modify_header(label = "**Zmienna ilościowa**") %>%
  as_kable_extra()
```
